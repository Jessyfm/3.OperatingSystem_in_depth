...
进程 : 一个具有一定独立功能的程序在一个数据集合上的一次动态执行过程.【动态执行】
...
结束状态(Exit): 一个进程正在从系统中消失时的状态, 这是因为进程结束或由于其它原因所导致.
	...
	<img width="576" alt="image" src="https://user-images.githubusercontent.com/44913612/161419641-0b8ebe2d-6e9d-4d6a-8d44-222423d8b737.png">
...
New → Ready: 当进程创建完成并初始化后, 一切就绪准备运行时, 变为就绪状态【很快】

Ready → Running  : 处于就绪态的进程被进程调度程序选中后, 就分配到处理机上来运行

Running → Exit   : 当进程表示它已经完成或者因出错, 当前运行进程会由操作系统作结束处理

Running → Ready  : 处于运行状态的进程在其运行过程中, 由于分配它的处理机时间片用完而让出处理机【保证进程公平】
...
<img width="473" alt="image" src="https://user-images.githubusercontent.com/44913612/161419768-5f5dc7d2-8e87-4bca-afe5-9f9e6b731632.png">

	
**两种挂起状态**

1.  阻塞挂起状态 : 进程在外存并等待某事件的出现;【本身进程处于阻塞状态】
2.  就绪挂起状态 : 进程在外存, 但只要进入内存, 即可运行.【进程本身处于就绪状态】
...
1.  从资源组合的角度: 进程把一组相关的资源组合起来,构成了一个资源平台(环境),包括地址空间(代码段,数据段),打开的文件等各种资源;【进程主要是管理资源】
...
-   线程所需的资源
	...
	<img width="517" alt="image" src="https://user-images.githubusercontent.com/44913612/161420325-ad708532-528f-47bf-8eb9-4e6dd2d32e76.png">
